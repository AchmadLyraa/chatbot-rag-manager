...
from app.utils.document_loader import load_document, LoadedDocument
...
def insert_chunks_to_db(file_id: str, chunks: list[Chunk]) -> None:
    conn = get_connection()
        ...
        with conn.cursor() as cur:
            ...
            for i, chunk in enumerate(chunks):
                ...
                cur.execute(
                    ...
                    (file_id, content, meta_data, embedding)
                    VALUES (%s, %s, %s, %s)
                    """,
                    (
                        file_id,
            ...
            cur.execute(
                "SELECT COUNT(*) FROM rag_chunks WHERE file_id = %s",
                (file_id,)
            )

            count = cur.fetchone()[0]

            print(f"\nTotal chunks for file: {count}")
...
def run_ingestion(file_id: str, file_path: str) -> dict:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "DELETE FROM rag_chunks WHERE file_id = %s",
                (file_id,)
            )
            conn.commit()
    ...
    insert_chunks_to_db(file_id, chunks)
